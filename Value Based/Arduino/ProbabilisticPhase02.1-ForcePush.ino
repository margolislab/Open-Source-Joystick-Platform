//=========================================================================
//====================== PROBABILISTIC TASK ===============================
//==================== PHASE 02.1: FORCE PUSH =============================
//=========================================================================

//===========================READ-ME=======================================
  //DESCRIPTION: Teach "pullers" how to push. Mouse receives reward when 
  //moving joystick either in push direction ONLY. A pull does nothing.
  
  //DISPLACEMENT THRESHOLD: 1.0mm
  
  //REWARD VOLUME: 12.5uL (!)
...
//=======================INITIAL SETUP=====================================
  ...
  //=====================TEMPORAL===============================
    ...
    const int OnDuration = 120; // ON time for pump (100ms for 10uL; 80ms for 7.5uL; 60ms 5uL; 40ms 2.5uL Last Calibration: 1/23/2022)
  ...
  //===================JOYSTICK-RELATED=========================
    ...
    byte RewardDirection = 1; //1 is push; 2 is pull
    ...
    const int Displ = 1750; //set joystick displacement threshold (in micrometers)
    const int JoyTempSensi=4000; //set temporal resolution for Joystick reading 400 - 250 Hz
...
void loop() {
  ...
  if (analogRead(A3) <400 && Started == 0 && BoxEntered == 1)
    {   
        Serial.println(F("ProbabilisticPhase02.1-ForcePush"));
  ...
  if (Initiated == 1)
  {
    ...
    if((A3MovingAverage > (JoystickBaselineA3 + Displ)) && Tripped == 0 && TrialDone == 0 && Rewarded == 0 && RewardDirection == 1 && TrialNumber >0)
    {
      Tripped = 1;
      TrippedPush = 1;
      LastDirection = 1;
    ...
    if((A3MovingAverage < (JoystickBaselineA3 - Displ)) && Tripped == 0 && TrialDone == 0 && Rewarded == 0 && RewardDirection ==2 && TrialNumber >0)
    {
      Tripped = 1;
      TrippedPull = 1;
      LastDirection = 2;
    ...
    //JOYSTICK MOVEMENT TRIPPED NOTHING FLAGS (for reading movements that trigger no consequence)
    if((A3MovingAverage > (JoystickBaselineA3 + Displ)) && Tripped == 0 && TrialDone == 0 && Rewarded == 0 && RewardDirection ==2 && TrialNumber >0)
    {
      TrippedNothing = 1;
      TrippedPush = 1;
    ...
    if (A3MovingAverage > (JoystickBaselineA3 - (0.25*Displ)) && TrippedNothing == 1 && TrippedPull ==1 && RewardDirection ==1 && TrialNumber >0)
    {
      TrippedNothing = 0;
    }
    
    if (A3MovingAverage < (JoystickBaselineA3 + (0.25*Displ)) && TrippedNothing == 1 && TrippedPush ==1 && RewardDirection ==2 && TrialNumber >0)
    {
      TrippedNothing = 0;
    }
    ...
    {
      if(TrippedCounter >=1 && TrippedPull ==1 && RewardDirection ==1 && TrialNumber >0)
      {
        ++Pulls;
        TrippedCounter=0;
        TrippedNothing = 0;
      ...
      if(TrippedCounter >=1 && TrippedPush ==1 && RewardDirection ==2 && TrialNumber >0)
      {
        ++Pushes;
        TrippedCounter=0;
        TrippedNothing = 0;
