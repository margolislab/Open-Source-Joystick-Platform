//=========================================================================
//======================= PROBABILISTIC TASK ==============================
//================= PHASE 05.1: SERIAL REVERSAL ===========================
//=========================================================================

//Updated: January 25, 2024

//===========================READ-ME=======================================
  //DESCRIPTION: Teach mice block architecture, i.e., there is a push block 
  //and a pull block, with blocks alternating on an uncued basis.
  //Contingencies are 100-0 for push-pull/pull-push and update every 17
  //trials + a geometrically distributed variable.
  
  //DISPLACEMENT THRESHOLD: 3mm
  
  //REWARD VOLUME: 8uL
  
  //CRITERION: >100 rewarded trials and >80% accuracy on 3 consecutive days. 
  //The fact that mice are doing blocks with uncued switches is the main
  //takeaway from this phase. As soon as they learn that, the subsequent
  //phases, in my experience, end up being a piece of cake. An additional
  //day/days just to make sure they're solid is a VERY WORTHY INVESTMENT.
  //I have spent weeks retraining mice that I advanced to Phase 06.1 too 
  //early.
...
//=======================INITIAL SETUP=====================================
  ...
  //=====================TEMPORAL===============================
    const int MaxTrialDuration = 10000; // Maximum Trial Duration is twenty seconds
  ...
  //=========EXPONENTIAL DISTRIBUTION PARAMETER SETTINGS========
    ...
    long PreCueInterval = 100; //conversion to long so arduino can parse
...
void loop() {
  ...
  { 
      Serial.println(F("ProbabilisticPhase05.1-SerialReversalNoTones"));
  ...
  if (Initiated == 1)
  {
    ...
    if(StartedBx == 1) 
    {
      ...
      //=========OUTCOME FLAGS============
      //REWARDED/UNREWARDED OUTCOME FLAG
      //Set to rewarded if movement is aftr go cue
      if (RewardDirection == 1)
      {
        if(TrippedPush == 1 && Rewarded == 0 && CuePlayed ==1 && TimeOut == 0 && TrialDone == 0) 
        {
          digitalWrite(LickometerLED,LOW);
          RewardMillis = millis();
          Rewarded = 1;
        ...
        if(TrippedPull == 1 && Unrewarded == 0 && CuePlayed == 1 && TimeOut ==0 && TrialDone == 0)
        {
          digitalWrite(LickometerLED,LOW);
      ...
      {
        if(TrippedPull == 1 && Rewarded == 0 && CuePlayed ==1 && TimeOut == 0) 
        {
          digitalWrite(LickometerLED,LOW);
          RewardMillis = millis();
          Rewarded = 1;
        ...
        if(TrippedPush == 1 && Unrewarded == 0 && CuePlayed == 1 && TimeOut ==0)
        {
          digitalWrite(LickometerLED,LOW);
          digitalWrite(BoxLight,LOW);
          UnrewardedMillis = millis();
    ...
    if(TrialDone ==1 && (millis()-(TrialDoneTime))>=(InterTrialInterval+(ServoDuration-ServoDuration)) && ServoMove ==0 && ServoBackPos ==1 && TimeOut == 0)
     {
        ...
        //start next block if BlockRewards >= BlockLength - only relevant after rewarded trial because this is the only thing that can trigger next block
        if (BlockRewards >= BlockLength && StartNextBlock ==0)
        {
          StartNextBlock = 1;
