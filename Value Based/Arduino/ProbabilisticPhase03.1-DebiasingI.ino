//=========================================================================
//====================== PROBABILISTIC TASK ===============================
//==================== PHASE 03.1: DEBIASING I ============================
//=========================================================================

//===========================READ-ME=======================================
  //DESCRIPTION: Teach mice to alternate between pushing and pulling for 
  //reward. Both directions are rewarded unless mouse consecutively moves 
  //joystick in one direction 5 times; in that case, that same side no 
  //longer will trigger reward. Movement of joystick in unrewarded direction 
  //does NOT trigger time-out
  ...
  //received >50 rewards. They need to learn the increased displacement
  //threshold in Phase 04.1 (Debiasing II). 
...
void loop() {
  ...
  if (analogRead(A3) <400 && Started == 0 && BoxEntered == 1)
    {   
        Serial.println(F("ProbabilisticPhase03.1-DebiasingI"));
  ...
  if (Initiated == 1)
  {
    ...
    if((A3MovingAverage > (JoystickBaselineA3 + Displ)) && Tripped == 0 && TrialDone == 0 && Rewarded == 0 && PushTally <5 && TrialNumber >0)
    {
      Tripped = 1;
      TrippedPush = 1;
      LastDirection = 1;
    ...
    if((A3MovingAverage < (JoystickBaselineA3 - Displ)) && Tripped == 0 && TrialDone == 0 && Rewarded == 0 && PullTally <5 && TrialNumber >0)
    {
      Tripped = 1;
      TrippedPull = 1;
      LastDirection = 2;
    ...
    if (A3MovingAverage > (JoystickBaselineA3 - (0.25*Displ)) && TrippedNothing == 1 && TrippedPull ==1 && TrialNumber >0)
    {
      TrippedNothing = 0;
    }
    
    if (A3MovingAverage < (JoystickBaselineA3 + (0.25*Displ)) && TrippedNothing == 1 && TrippedPush ==1 && TrialNumber >0)
    {
      TrippedNothing = 0;
    }
